  ...
  <a-modal
    ...
    <a-layout class="user-info">
      <a-layout-sider class="user-info-side">
        <!-- <a-avatar shape="square" :size="115" icon="user" :src="`static/avatar/${userInfoData.pic1}`"/> -->
        <a-avatar shape="square" :size="115" icon="user" :src="`${userInfoData.pic1}`"/>
        <!-- <p>标题：{{userInfoData.title ? userInfoData.title : '暂无信息'}}</p>
        <p>户型：{{getunitype(userInfoData.unittype)}}</p>
        <p>楼层：{{userInfoData.floor ? userInfoData.floor : '暂未信息'}}</p>
        <p>电梯：{{getele(userInfoData.elevator)}}</p>
        <p>面积：{{userInfoData.acreage ? userInfoData.acreage : '暂无信息'}}</p>
        <p>装修：{{getdero(userInfoData.decoration)}}</p> -->
      </a-layout-sider>
...
export default {
  ...
  methods: {
    ...
    admit () {
      this.$put('FrontController/updateRent', {
        rentid: this.userInfoData.rentid,
        isChecked: 1
      }).then(
